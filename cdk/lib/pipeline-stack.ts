...
export class PipelineStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: PipelineStackProps) {
    ...
    const buildRole = new iam.Role(this, 'CodeBuildRole', {
      ...
      description: 'CodeBuild role for Spring Security docs indexing pipeline',
    });
    ...
    const placeholderSpec = codebuild.BuildSpec.fromObject({
      version: '0.2',
      env: {
        variables: {
      ...
      phases: {
        build: {
          commands: [
            'echo "Placeholder: replace with Antora build + chunking + Bedrock embed + S3 Vectors PutVectors pipeline"',
            'echo "Targets: $CONTENT_BUCKET / $VECTOR_BUCKET / $VECTOR_INDEX / $CHUNKS_TABLE / $KEYWORDS_TABLE"',
          ],
        },
      },
    });
    ...
    new CfnOutput(this, 'BuildProjectName', { value: this.buildProject.projectName });
  }
}
